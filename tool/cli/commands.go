...
func RegisterCommands(app *cobra.Command, c *client.Client) {
	...
	command = &cobra.Command{
		Use:   "resolve",
		Short: `Resolves IPs from a specified hostname`,
	}
	tmp1 := new(ResolveResolveCommand)
	sub = &cobra.Command{
		Use:   `resolve ["/api/HOSTNAME"]`,
		Short: ``,
...
func (cmd *ResolveResolveCommand) Run(c *client.Client, args []string) error {
	...
	if len(args) > 0 {
		...
		path = fmt.Sprintf("/api/%v", url.QueryEscape(cmd.Hostname))
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.ResolveResolve(ctx, path)
	if err != nil {
...
func (cmd *ResolveResolveCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	var hostname string
	cc.Flags().StringVar(&cmd.Hostname, "hostname", hostname, `Hostname to resolve`)
}
